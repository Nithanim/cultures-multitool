...
public class LibFilePath implements Path {
  ...
  public LibFilePath subpath(int beginIndex, int endIndex) {
    initOffsets();
    if ((beginIndex < 0)
        || (beginIndex >= this.offsets.length)
        || (endIndex > this.offsets.length)
        || (beginIndex >= endIndex)) {
  ...
  public LibFilePath relativize(Path other) {
    ...
    if (nbCommon < nbNames2) {
      System.arraycopy(
          p2.path, p2.offsets[nbCommon], result, idx, p2.path.length - p2.offsets[nbCommon]);
    }
    return new LibFilePath(fileSystem, result);
  }
    ...
    for (int j = 0; j < path.length; j++) {
      ...
      if (k == 0) {
        throw new InvalidPathException(
            new String(path, StandardCharsets.ISO_8859_1), "Path: nul character not allowed");
      }
      i = k;
    }
    return path;
